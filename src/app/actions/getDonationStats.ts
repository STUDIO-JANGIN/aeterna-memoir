...
/**
 * Returns count and list of completed platform_tip payments for the event.
 * list[].displayLabel is masked for social proof.
 */
export async function getDonationStatsAction(slug: string): Promise<DonationStatsResult> {
  ...
  const list = (rows ?? []).map((r) => {
    ...
    return { displayLabel: `${label} added a small gesture of support.` }
  })
...
function maskDonorLabel(email: string | null): string {
  if (!email || typeof email !== "string") return "Anonymous supporter"
  const at = email.indexOf("@")
  if (at <= 0) return "Anonymous supporter"
  const local = email.slice(0, at).trim()
  if (!local) return "Anonymous supporter"
  const first = local[0]
  return first + "**"
}
