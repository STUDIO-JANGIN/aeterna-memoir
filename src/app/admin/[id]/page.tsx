...
import { getEventSlugByIdAction } from "@/app/actions/getEventSlugById"
...
export default function AdminIdRedirectPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    getEventSlugByIdAction(id).then((slug) => {
      if (cancelled) return
      if (!slug) {
        setStatus("not-found")
        return
  ...
  if (status === "loading") {
    return (
      <div className="min-h-dvh flex items-center justify-center bg-[var(--aeterna-charcoal)] font-sans text-sm text-[var(--aeterna-gold-muted)] tracking-[0.1em] uppercase">
  ...
  if (status === "not-found") {
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center bg-[var(--aeterna-charcoal)] font-sans px-6 text-center text-white">
