import type { AppStrings, DeepPartial } from "@/lib/appTranslations"
import { splitMsToDHMS } from "@/lib/memorialCountdownParts"

/** French — sophisticated phrasing; Sanctuaire / héritage tone */
export const FR_APP_REST: DeepPartial<AppStrings> = {
  find: { open: "Ouvrir" },
  myMemorial: {
    deletePlaceholder: "supprimer",
  },
  createNew: {
    preparing: "Préparation de votre sanctuaire…",
    kicker: "Nouveau mémorial",
    title: "Quelques mots sur votre proche",
    body: "Ces informations nous aident à composer le sanctuaire et les détails de cérémonie. Vous pourrez tout ajuster depuis l’espace famille.",
    errName: "Veuillez indiquer le nom de votre proche.",
    errCeremony: "Veuillez choisir le mois, le jour et l’heure de la cérémonie.",
    errCreate: "Le sanctuaire n’a pas pu être créé. Veuillez réessayer.",
    labelName: "Nom",
    phName: "Nom complet",
    labelBirth: "Date de naissance",
    labelDeath: "Date de décès",
    dateFormatHint: "Format : mm/jj/aaaa",
    labelPortrait: "Portrait",
    addSpark: "Offrir une étincelle de souvenir",
    portraitHint:
      "Visible sur le sanctuaire et dans la présentation — un portrait clair aide chacun à le reconnaître.",
    selectedFile: "Fichier :",
    labelLocation: "Lieu de cérémonie",
    phLocation: "Lieu, chapelle ou adresse",
    searchingPlaces: "Recherche de lieux à proximité…",
    locationMockError: "Suggestions indisponibles — saisissez l’adresse manuellement",
    labelServiceTime: "Heure de la cérémonie",
    month: "Mois",
    date: "Jour",
    selectTime: "Choisir l’heure",
    serviceTimeNote: "Les invités verront ces informations telles quelles sur le sanctuaire.",
    labelContribution: "Lien de soutien (facultatif)",
    phContribution: "ex. cagnotte, PayPal, virement",
    contributionHint:
      "Facultatif. Toute page de collecte apparaîtra comme « Soutenir la famille » sur le sanctuaire.",
    labelMusic: "Musique d’ambiance (facultatif)",
    phMusic: "Lien YouTube ou URL audio",
    musicHint: "Facultatif. Souvent un seul lien vers une chère mélodie.",
    footerNote:
      "Ensuite s’ouvre l’espace famille : vous pourrez recueillir les hommages et partager le lien.",
    submit: "Poursuivre l’histoire",
    submitting: "Création…",
  },
  pricingCurrencyNote: {
    usd: "Prix en dollars (USD).",
    krw: "Facturation en won coréen (KRW).",
    jpy: "Facturation en yen japonais (JPY).",
    sar: "Facturation en riyal saoudien (SAR).",
  },
  memorial: {
    loadSyncing: "Synchronisation du sanctuaire…",
    loadLoading: "Ouverture du sanctuaire…",
    syncHint: "Nous confirmons votre sanctuaire sur nos serveurs — un instant après le paiement.",
    notFound: "Sanctuaire introuvable.",
    addMemory: "Ajouter un souvenir",
    share: "Partager",
    admin: "Admin",
    photoWindow: "Fenêtre de partage ·",
    linkCopied: "Lien copié.",
    premiumRestoreTitle: "Rouvrir avec Premium",
    premiumRestoreBody:
      "La fenêtre de collecte s’est refermée ; les souvenirs sont protégés. Passez à Premium pour les rouvrir.",
    paymentsSoonTitle: "Paiement bientôt disponible",
    paymentsSoonBody: "Un paiement sécurisé ouvrira bientôt tous les souvenirs.",
    adminForbidden: "Vous n’avez pas accès aux réglages.",
    donationThankYou: "Merci pour votre soutien délicat.",
    memoryReceivedTitle: "Souvenir reçu",
    memoryReceivedBody:
      "Votre souvenir a été soumis et sera affiché après approbation de l'administrateur. Une fois public, les autres pourront aimer et commenter votre message.",
    emailPlaceholder: "E-mail",
    saving: "Enregistrement…",
    notifyMe: "Me prévenir",
    shareModalTitle: "Share this memorial",
    shareModalBody:
      "Share this Memorial. Invite family and friends to visit and share memories together.",
    whatsApp: "WhatsApp",
    message: "Message",
    copyLink: "Copier le lien",
    shareChannelEmail: "E-mail",
    filmAria: "Clips hommage IA",
    videoUnsupported: "Votre navigateur ne lit pas la vidéo.",
    filmLabel: "Clips",
    redirectCheckout: "Redirection vers le paiement…",
    downloadFilm: "Télécharger les clips en haute définition",
    unlockMemories: "Libérer tous les souvenirs",
    premium: "Premium",
    locked: "Protégé",
    viewStoryAria: "Lire le récit",
    storyDrawerTitle: "Souvenir",
    storyDrawerNoImage: "Pas d’image",
    storyDrawerAnonymous: "Anonyme",
    storyDrawerGuestName: "Invité",
    storyDrawerHeartAriaRemove: "Retirer le cœur",
    storyDrawerHeartAriaAdd: "Aimer ce souvenir",
    storyDrawerHeartsLabel: "Cœurs",
    storyDrawerAiFilmHint: "Les photos les plus aimées peuvent apparaître dans le film.",
    storyDrawerCommentsHeading: "Partager un souvenir",
    storyDrawerCommentsLoading: "Chargement…",
    storyDrawerNoCommentsYet: "Pas encore de messages — vous pouvez partager un souvenir ci-dessous.",
    storyDrawerCommentHeartAriaRemove: "Retirer le cœur de ce message",
    storyDrawerCommentHeartAriaAdd: "Aimer ce message",
    storyDrawerCommentHeartTitleRemove: "Retirer le cœur",
    storyDrawerCommentHeartTitleAdd: "Cœur",
    storyDrawerYourNamePlaceholder: "Votre nom (facultatif)",
    storyDrawerComposerPlaceholder: "Quelques mots…",
    storyDrawerComposerLabel: "Partager un souvenir",
    storyDrawerSendAria: "Envoyer",
    donationSupportCta: "soutien · voir les coordonnées",
    donationStatus: "État du soutien",
    donationSoFar: (count: number) =>
      `Déjà ${count} personnes ont témoigné leur soutien à la famille.`,
    shareMemoryTitle: "Offrir un souvenir",
    stepCounter: (n: number) => `${n} / 3`,
    formNameTitle: "Votre nom ?",
    formNamePh: "Votre nom",
    formPhotoTitle: "Ajoutez une image.",
    changePhoto: "Changer la photo",
    tapAddMemory: "Toucher pour ajouter",
    dragHere: "ou glisser ici",
    formStoryTitle: "Quelle histoire se cache derrière cette image ?",
    formStoryPh: "Un voyage, un instant tranquille, un sourire qui demeure…",
    formStoryPremium:
      "Votre photo pourrait figurer dans l’hommage en cinq clips (~10 s chacun), style « images mouvantes ».",
    formStoryFree: "Merci d’avoir partagé ce trésor.",
    sending: "Envoi…",
    shareThisMemory: "Partager ce souvenir",
    continueTheStoryBtn: "Poursuivre l’histoire",
    restorePremium: "Rouvrir avec Premium",
    collectionClosed: "La collecte des photos est close",
    filmCraftedTitle: "Vos clips hommage se composent",
    filmCraftedSubtitle: "Vos souvenirs se tissent en un hommage durable",
    collectionClosedGalleryNote:
      "Merci pour les souvenirs déposés ici — la galerie demeure visible ci-dessous.",
    notifyPlaceholder: "Votre e-mail",
    notifyFilmTitle: "Prévenez-moi lorsque les clips sont prêts",
    notifyFilmThanks: "Merci. Nous vous écrirons dès qu’ils seront prêts.",
    noMemoriesYet: "Aucun souvenir partagé pour le moment.",
    eternalFilmClipPhotoLimitAlert:
      "Pour une qualité IA optimale, veuillez utiliser 1 à 2 photos par clip.",
    eternalFilmUploaderChoose: "Choisir des photos",
    preserveLegacyHeader: "Préserver cet héritage",
    trialGatheringTimerLabel: "Temps restant pour recueillir les souvenirs",
    trialCountdownFromMs: (ms: number) => {
      const { d, h, m, s } = splitMsToDHMS(ms)
      const p = (n: number) => String(n).padStart(2, "0")
      return `${p(d)}j : ${p(h)}h : ${p(m)}min : ${p(s)}s`
    },
    trialUpgradePart1: "Pour garder ces souvenirs vivants à jamais, veuillez ",
    trialUpgradeLinkLabel: "mettre à niveau",
    trialUpgradePart2:
      " sous 7 jours. Passé ce délai, le sanctuaire se fermera délicatement pour protéger la confidentialité des données.",
    memorialUpgradeAnchorIntro:
      "Terminez le paiement sécurisé ci-dessous pour préserver ce sanctuaire et conserver tous les souvenirs partagés.",
    adminLoadingTitle: "Chargement",
    adminLoadingSubtitle: "Préparation de votre tableau de bord…",
    adminAccessRestrictedTitle: "Accès restreint",
    adminAccessRestrictedBody: "Vous n’avez pas accès à ce tableau de bord.",
    adminBackToHome: "Retour à l’accueil",
    adminNotFoundTitle: "Introuvable",
    adminNotFoundBody: "Ce sanctuaire n’a pas pu être chargé.",
    adminDashboardKicker: "Tableau de bord",
    adminDashboardWelcome:
      "Organisez les souvenirs, protégez l'héritage et partagez ce sanctuaire. Gérez le profil et les médias à tout moment dans les Paramètres.",
    adminEdit: "Modifier",
    adminSharePdfInvitation: "Invitation PDF",
    adminPdfGenerating: "Génération…",
    adminInvitationSheetTitle: "Partager et télécharger",
    adminInvitationSheetPreparing: "Préparation de l’invitation…",
    adminInvitationShareInstagram: "Partager via Instagram",
    adminInvitationShareLine: "Partager via LINE",
    adminInvitationShareWhatsApp: "Partager via WhatsApp",
    adminInvitationNativeShare: "Partager avec…",
    adminInvitationNativeShareHint: "Messages, Mail, AirDrop, etc.",
    adminInvitationDownload: "Télécharger le PDF",
    adminInvitationClose: "Fermer",
    adminInvitationError: "Impossible de préparer le PDF.",
    adminInvitationCopied: "Lien copié.",
    adminInvitationShareText: (name: string) => `Invitation commémorative — ${name || "votre proche"}`,
    adminBackToFeed: "Retour au flux",
    adminCurrentPlan: "Forfait actuel",
    adminTierLabelFree: "Gratuit",
    adminTierLabelPlus: "Plus",
    adminTierLabelPremium: "Premium",
    adminContributionsCollected: (count: number) => `${count} contributions sincères collectées`,
    adminPremiumStatusLine: (count: number) =>
      `Forfait actuel / Premium / ${count} contributions sincères collectées`,
    adminPremiumAiTitle: "Premium : génération Eternal Film (5 clips)",
    adminPremiumAiDescription:
      "Chaque clip dure environ 10 secondes. Luma AI y ajoute un mouvement doux et magique.\nPour chaque clip, choisissez 1 à 2 photos. Nous intégrons l’histoire de chaque photo et les mots des visiteurs (commentaires) dans le prompt pour des instants les plus chaleureux.",
    adminPremiumFilmSelectionSummary: (selected: number, max: number) =>
      `Photos sélectionnées : ${selected} / ${max} (maximum ${max})`,
    adminPremiumGenerateFilmCta: "Générer le prochain clip hommage (~10 s)",
    adminPremiumSelectMinGuide: "Sélectionnez au moins une photo pour continuer.",
    adminPremiumFooterTagline: "Aeterna Memoir — Préserver vos précieux souvenirs.",
    adminPremiumMaxPhotosHint: (max: number) =>
      `Vous pouvez choisir jusqu’à ${max} photos pour l’hommage.`,
    adminPremiumFilmSelectRangeError: (min: number, max: number) =>
      `Sélectionnez entre ${min} et ${max} photos approuvées.`,
    adminPremiumTributeLiveBody: "Votre hommage est en ligne sur le mémorial et prêt à être partagé.",
    adminPremiumPreviewOnMemorialCta: "Aperçu sur le mémorial",
    adminPremiumFilmCraftingTitle:
      "Notre IA prépare votre hommage… cela peut prendre 1 à 2 minutes.",
    adminPremiumFilmCraftingSubtitle:
      "Vous pouvez quitter cette page — nous mettrons le mémorial à jour lorsque le clip sera prêt. Cette page se rafraîchit automatiquement.",
    adminPremiumFilmFailed:
      "Un problème est survenu pendant le rendu. Contactez le support — nous pouvons rétablir votre crédit clip et vous aider à réessayer.",
    adminPremiumApprovePhotosFirst:
      "Approuvez d’abord les photos des invités dans la section Souvenirs ci-dessous, puis revenez ici pour créer vos clips.",
    adminPremiumRemoveFromFilmAria: "Retirer de la sélection hommage IA",
    adminPremiumIncludeInFilmAria: "Inclure dans la sélection hommage IA",
    adminPremiumClipsCompletedStatus: (completed: number, total: number) =>
      `Clips terminés : ${completed} / ${total}`,
    adminPremiumCompletedClipsLabel: "Vos clips hommage",
    adminPremiumClipLabel: (clipOneIndexed: number, totalClips: number) =>
      `Chapitre ${clipOneIndexed} · ~10 s · ${totalClips} au total`,
    adminPremiumAllClipsComplete:
      "Les cinq chapitres sont prêts. Retrouvez-les ci-dessus ou sur le mémorial public.",
    adminPremiumNoClipCredits: "Plus de crédits clip. Contactez le support si besoin.",
    adminPremiumAiWaitlistTitle: "Vous êtes sur la liste d’attente",
    adminPremiumAiWaitlistBody:
      "La génération de clips hommage par IA n’est pas encore disponible sur le serveur. Vous avez été inscrit·e sur notre liste d’attente — nous vous contacterons dès que ce sera prêt. Pour toute question : hoon@aya.yale.edu",
    adminPreserveForeverCta: "Préserver à jamais",
    adminEternalFilmCta: "Film éternel",
    adminPlusPreservedBody:
      "Votre espace commémoratif est préservé. Chaque récit et chaque photo resteront ici aussi longtemps que vous le souhaiterez.",
    adminProcessing: "Traitement…",
    adminMemoriesSectionTitle: "Souvenirs",
    adminTabPending: (count: number) => `En attente (${count})`,
    adminTabApproved: (count: number) => `Approuvés (${count})`,
    adminStoryApprove: "Approuver",
    adminStoryDelete: "Supprimer",
    adminStoryUnapprove: "Retirer l'approbation",
    adminStoryDeleteAria: "Supprimer le souvenir",
    adminStoryDeletePermanentAria: "Supprimer définitivement le souvenir",
    adminApprovedEmpty:
      "Aucun souvenir approuvé pour l'instant. Examinez les soumissions en attente pour nourrir le sanctuaire.",
    adminDeleteMemoryConfirm:
      "Supprimer définitivement ce souvenir ? Cette action est irréversible.",
    adminToastStoryMovedToPending: "La photo est repassée en attente.",
    upgradePremiumCta: "Passer à Premium",
    upgradePremiumTail: "pour des clips hommage IA sur vos futurs sanctuaires.",
    recentSupportAria: "Soutiens récents",
    donationStatusAria: "État du soutien",
    close: "Fermer",
    returnHome: "Retour à l’accueil",
    errors: {
      nameRequired: "Indiquez votre nom.",
      photoRequired: "Choisissez une photo.",
      storyRequired: "Ajoutez votre nom et le récit lié à la photo.",
      emailRequired: "Indiquez votre e-mail.",
      subscribeFailed: "L’inscription a échoué pour l’instant. Réessayez bientôt.",
      submitFailed: "Envoi impossible.",
      memorialNotFound: "Sanctuaire introuvable.",
      loadFailed: "Une erreur est survenue au chargement.",
      checkoutFailed: "Impossible de lancer le paiement.",
      donationCheckoutFailed: "Le paiement n’a pas démarré. Réessayez.",
    },
  },
  adminProfilePage: {
    backToAdmin: "Retour à l’admin",
    pageTitle: "Paramètres du profil",
    editProfileSection: "Modifier le profil",
    nameLabel: "Nom",
    namePlaceholder: "Nom de la personne aimée",
    birthDateLabel: "Date de naissance",
    birthDatePlaceholder: "ex. 1950-01-15",
    dateOfPassingLabel: "Date de décès",
    dateOfPassingPlaceholder: "ex. 2024-03-01",
    locationLabel: "Lieu",
    locationPlaceholder: "Ville, lieu, etc.",
    ceremonyTimeLabel: "Heure de la cérémonie",
    ceremonyTimePlaceholder: "ex. 15 mars 2024 à 14h",
    invitationContactPhoneLabel: "Téléphone de contact (invitation)",
    invitationContactPhonePlaceholder: "Facultatif — affiché sur le PDF imprimé",
    remembranceMessageLabel: "Message commémoratif",
    remembrancePlaceholder: "Court message sur le sanctuaire et l’invitation (facultatif)",
    remembranceHint:
      "Affiché sur la page publique et l’invitation. Modifiable à tout moment.",
    condolenceAccountLabel: "Détails du compte de condoléances",
    condolencePlaceholder: "Banque, numéro de compte, titulaire, etc.",
    saveProfile: "Enregistrer le profil",
    saving: "Enregistrement…",
    generateQrInvitation: "Générer l’invitation QR",
    generating: "Génération…",
    loading: "Chargement…",
    memorialNotFound: "Sanctuaire introuvable.",
    invalidSlug: "URL invalide : identifiant manquant.",
    eventNotFound: "Événement introuvable.",
    saveFailed: "Enregistrement impossible.",
    generatePdfFailed: "Impossible de générer le PDF d’invitation.",
    openInvitationWithLocale: (localeUpper: string) => `Ouvrir l’invitation (${localeUpper})`,
  },
  createWizard: {
    stepOf: "Étape {current} sur {total}",
    resumeToast: "Votre progression est enregistrée — reprenez ici.",
    welcomeSacred: "Préparation d'un havre de paix pour vos souvenirs.",
    paymentPending: "Le paiement n’est pas terminé.",
    paymentPendingBold: "Poursuivre l’histoire",
    dismiss: "Fermer",
    whoHonoringTitle: "À qui rendons-nous hommage ?",
    whoHonoringSubtitle: "Un espace paisible pour se souvenir d'un être aimé.",
    honorPerson: "Un être cher",
    honorPet: "Un compagnon",
    step1Title: "Son nom",
    step1Subtitle: "Quel nom souhaitez-vous afficher ?",
    step1Ph: "Nom",
    step2Title: "Un visage à chérir",
    step2HintPhoto: "Faites glisser la photo dans le cercle pour l’ajuster.",
    step2HintNoPhoto: "Une image aide à se souvenir. Touchez le cercle pour l’ajouter.",
    chooseDifferentPhoto: "Choisir une autre photo",
    addSparkAria: "Ajouter une étincelle de mémoire",
    addSparkLabel: "Ajouter une étincelle de mémoire",
    step3Title: "Honorer son parcours",
    step3Subtitle: "Les dates exactes sont idéales ; une année seule suffit parfois.",
    born: "Date de naissance",
    atRest: "Date de décès",
    datePassingBeforeBirth:
      "La date de décès ne peut pas être antérieure à la date de naissance. Veuillez vérifier les dates.",
    step4Title: "Quelques mots à son sujet",
    step4Body: "Partagez un court hommage.",
    remembranceLabel: "Hommage",
    remembrancePh: "Facultatif — une phrase ou davantage.",
    enhanceWithAi: "Améliorer avec l’IA",
    enhanceGenerating: "Affinage…",
    enhanceChooseVersion: "Choisir une version",
    enhanceUseThis: "Utiliser celle-ci",
    enhanceWriteFirst: "Écrivez d’abord quelques lignes.",
    enhanceTooLong: "Texte trop long — raccourcissez puis réessayez.",
    enhanceErrorGeneric: "Une erreur est survenue. Réessayez.",
    enhanceOptionPoetic: "Poétique",
    enhanceOptionFormal: "Solennel",
    enhanceOptionWarm: "Tendre",
    enhanceRefine: "Affiner",
    enhanceRefining: "Affinage…",
    step5Title: "Y aura-t-il une cérémonie commémorative ?",
    step5Body:
      "Si oui, vous pourrez indiquer lieu, date et heure à l’étape suivante.",
    yes: "Oui",
    no: "Non",
    step6Title: "Détails de la cérémonie",
    step6Body: "Renseignez les détails ou revenez plus tard.",
    location: "Lieu",
    date: "Date",
    time: "Heure",
    phLocation: "Lieu, adresse ou ville",
    hour: "Heure",
    min: "Min",
    amPm: "AM / PM",
    step7Title: "Soutien à la famille",
    step7Body: "Veuillez ajouter un lien de soutien.",
    support: "Lien de soutien",
    phFund: "Lien de cagnotte ou association",
    step8Title: "Validez votre sanctuaire",
    step8Body:
      "Connectez-vous pour sécuriser ce sanctuaire et le modifier à tout moment.",
    signedIn: "Connecté",
    checkingAccount: "Vérification du compte…",
    nextChoosePlan: "Suivant : choisissez comment vous souhaitez préserver leur page",
    continueWithGoogle: "Poursuivre l’histoire",
    almostThereTitle: "Presque terminé",
    almostThereBody:
      "Lorsque vous êtes prêt, nous ouvrons le sanctuaire — ou le paiement si vous choisissez une offre.",
    yourSelectedPlan: "Votre choix",
    comingSoonTag: "Bientôt",
    changePlanPrompt: "Souhaitez-vous changer d’offre ?",
    backToSummary: "← Retour au résumé",
    keepTheirLightTitle: "Garder sa lumière",
    keepTheirLightBodyLocked:
      "Choisissez l’offre qui vous convient — vous pourrez l’ajuster selon votre compte.",
    keepTheirLightBodyOpen: "Un choix serein.",
    invitationReadyKicker: "Invitation prête",
    invitationTitle: (name: string) => `Le sanctuaire de ${name} est prêt`,
    invitationBody: "Partagez une invitation délicate.",
    viewMemorial: "Voir le sanctuaire",
    creating: "Création…",
    checkingAccountBtn: "Vérification…",
    plans: {
      free: {
        title: "Fenêtre de Mémoire",
        tagline: "7 jours pour réunir les premiers souvenirs. Un début doux et serein.",
        b1: "Sept jours pour que famille et amis déposent photos et récits",
        b2: "Passez à une offre supérieure pour graver le sanctuaire dans le temps",
        tierSub: "Sept jours. Début apaisé.",
      },
      plus: {
        title: "Héritage Éternel",
        tagline: "Préservez chaque photo et chaque récit pour toujours. Sans date d'expiration.",
        b1: "Souvenirs conservés sans fin",
        b2: "Un sanctuaire partageable, permanent",
        tierSub: "Pour toujours. Sans expiration.",
      },
      premium: {
        title: "Le Film Éternel",
        tagline:
          "Tous les avantages d'Héritage Éternel, avec cinq clips hommage IA (~10 s chacun)",
        b1: "Tout l’héritage éternel",
        b2: "Cinq clips « images mouvantes » pour retrouver leur présence — bientôt disponible",
        tierSub: "5× ~10 s · Bientôt disponible",
      },
    },
  },
}
