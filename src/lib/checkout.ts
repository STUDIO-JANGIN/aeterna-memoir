 ...
 * Includes regional wallets (e.g. Kakao, Naver, Samsung) where Stripe supports them.
 */
...
/** Fallback when account/region does not support the full list */
 ...
 */
export function getDonationProductCopy(_locale: string): {
  name: string
  description: string
} {
  return {
